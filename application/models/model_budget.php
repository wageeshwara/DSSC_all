...
class model_budget extends CI_Model {
	...
	{
		$query=$this->db->query("SELECT * FROM budget WHERE Active=1");
		if($query->num_rows()>0)
			{
				return $query->result();
				}
	...
	{
		$query=$this->db->query("SELECT * FROM budget WHERE Active=1 AND UId='$id'");
		if($query->num_rows()>0)
			{
				return $query->result();
				}
	...
	function getSpecificBudget($id)
	{
		$query=$this->db->query("SELECT * FROM budget WHERE Active=1 AND EventId='$id'");
		if($query->num_rows()>0)
			{
				return $query->result();
				}
